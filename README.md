...
mac 启动崩溃修复，编排判据收口，外部会话接管打通远程：

**🐛 问题修复**

- **mac 上启动即闪退（SIGABRT）** — 关掉 AppKit 窗口恢复，堵住触发路径。决定性线索在系统弹框「恢复窗口时退出」，不在崩溃栈里
- **Codex 聊天模式在 mac 上起不来（第二例）** — 进程树的 cwd 钉在 `.app` 包内，更新一次就失效
- **编排：报错与重试被判成「跑完了」** — 既误唤醒总控，还把下一条消息投进正在重试的终端；failed 现在明说无产出
- **编排：`contextPercent` 恒为 null** — 探针的门控问晚了一步，测出来的值又推不出去
- **编排：聊天模式的会话认不出自己** — `AICODER_SESSION_ID` 被整前缀清理误伤

**🚀 新功能**

- **接管外部会话打通远程** — 九家 CLI 全部可在服务器上发现并接管，不再限于本机
- **聊天会话数出厂不限制** — 进程生命周期交回用户手里

**✨ 体验优化**

- **「回到底部」改成离底才出现的胶囊浮标** — 不再和右下角那排 FAB 抢地方
- **watchdog 日志落文件** — GUI 启动下 `Target::Stderr` 等于全丢，崩溃后查不到任何线索
- **stdio 输出安全化** — 裸 `println!` / `eprintln!` 换成 log 宏，避免管道已关闭时炸在输出上
...
- **订阅额度搬进软件** — Claude / Codex / Grok 的额度不再只能靠 StatusLine 插件看，状态栏常驻显示，支持多账号与远程账号；额度恢复、用量预警、凭据失效都会主动通知
- **通知支持邮件** — 填邮箱 + 授权码即可，服务器自动识别；正文改为 HTML 版式
- **状态栏跟随终端** — 在终端里敲 `/model` 换模型后，状态栏显示会跟着变
- **编排增强** — 总控能确定自己是谁、派过哪些子会话，上下文被压缩后也能找回
- **验收标准自动建议** — 按项目形态与任务描述，给出可执行的验收清单

**🐛 问题修复**

- **根治「新窗口白屏且整个应用关不掉」** — WebView2 运行期换版时新建窗口会顶掉事件循环，已补建窗闸门与 watchdog 第三条卡死判据
- **修复打包版点任务重启，第一次只把任务关掉了** — 生产环境取不到进程 PID，导致「等旧进程退干净再启动」的保护被跳过
- **终端提示改走浮层** — 编排按键、额度轮换等提示不再写进终端缓冲区，不会再把输入框挤乱

**✨ 体验优化**

- 配置档案工具栏改为平铺，迁移档案等动作不再藏在「更多」里
- 会话删除加确认气泡，标签列表可一键移除已自动关闭的会话
- 状态栏按钮内边距收窄，上下文占比标签改用 `ctx` 简写
...
├── .gitignore          # Git 忽略规则
├── screenshots/        # README 里的应用预览图
└── releases-mobile/    # 移动端伴侣 Android APK + AAB（独立版本号）
    └── mobile-vX.Y.Z/  # 每版含 universal-release APK + AAB
```

> **桌面端产物不在仓库里**（自 v5.8.3 起）。每版 12 个文件改放
> [Releases 附件](https://github.com/bkywksj/aicoder-release/releases)，
